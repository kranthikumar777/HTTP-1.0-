...
int main()
{
	...
	int serv_sock, new_sock,sock_act,port_num,master_check,max_clients,n,rw_check,getaddr_check,i,j,k,a,d,conn_act,recv_act_check,LRU_check;//,LRU_pointer;
	LRU_check=0;
	//LRU_pointer=0;
	...
	//Update FD Set
		...
		for(i=0; i<=n;i++)
		{
			if(FD_ISSET(i, &read_fds))	//Data received on the ith connection
			{
				...
				printf("Sending stored copy of document \n");
				LRU_check=0;
				}
				else	
					{
